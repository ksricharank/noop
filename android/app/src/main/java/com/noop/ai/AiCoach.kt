...
class AiCoach(
    ...
    private fun httpError(provider: AiProvider, code: Int, body: String): Exception {
        ...
        // A rejected key is NOT retryable: a fallback model would be turned away by the same key, so
        // the rejection is surfaced (and drives the key-repair affordance) instead of being retried.
        // Everything else the retry could plausibly fix stays retryable. Parity with Swift.
        return if (isKeyRejection(code)) AiKeyRejectedException(message)
               else CoachRetryableException(message)
    }
