        ...
        case .assistant:
            ...
            HStack {
                VStack(alignment: .leading, spacing: 4) {
                    ...
                    // Attribution, shown ONLY when a fallback answered. A reply produced by a model
                    // other than the one in the picker would otherwise read as the chosen model's work.
                    // Absent on an ordinary reply, so the transcript stays quiet when there is nothing
                    // to disclose — the header pill already names the selected model.
                    if let fallback = message.fallbackModel {
                        Text("generated by \(fallback)")
                            .font(StrandFont.footnote)
                            .foregroundStyle(StrandPalette.textTertiary)
            ...
            .accessibilityElement(children: .combine)
            .accessibilityLabel(message.fallbackModel.map {
                "Coach said, generated by \($0): \(message.text)"
